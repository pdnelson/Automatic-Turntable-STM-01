...
void StmStepper::setSpeed(uint8_t speedRpm) {
    this->topSpeedTimeBetweenStepsMicros = (60000000L / STEPPER_STEPS_PER_REVOLUTION / speedRpm) * 1000L;
}
...
bool StmStepper::step(unsigned long clockMicros) {
...
void StmStepper::performStep() {
    ...
    if(currentStep == 4 || currentStep == 255) {
        currentStep = 0;
    }
